...
AddEventHandler('esx_customui:updateStatus', function(status)
  local hunger = status[1].percent
  local thirst = status[2].percent
  SendNUIMessage({action = 'updateHunger', data = hunger})
  SendNUIMessage({action = 'updateThirst', data = thirst})
end)
